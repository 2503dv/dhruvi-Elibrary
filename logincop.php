...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // $sql = "SELECT *FROM signdhruvi where username='$username' AND usertype='$usertype'";
    // $sql = "SELECT * FROM `signdhruvi` WHERE username='$username'";
    // $sql = "SELECT * FROM `signdhruvi` WHERE `usertype` LIKE '$usertype' AND `username` LIKE '$username' AND `password` LIKE '$password'";
    $sql = "SELECT * FROM `signdhruvi` WHERE `usertype` LIKE '$usertype' AND `username` LIKE '$username' ";
    $result = mysqli_query($conn, $sql);
    $num = mysqli_num_rows($result);
    if ($num == 1) {
        while ($row = mysqli_fetch_assoc($result)) {
            // $x = password_verify($password1, $row['password']);
            // $showError = $x;
            // $showError = $row['password'];
            $x = password_verify($password1, $row['password']);
            // $showError = $x;
            // $hash = $row['password'];
            if ($x) {
                ...
                // header("location: backdb.html");
                header("location: backdb.php");
            } else {
                // $showError = $x;
                $showError = "invalid password ! " . $password1;
            }
        }
    } else {
        $showError = "invalid try !";
    }
}
